...
const statusForCoverage = (v: number) => v >= 90 ? "EXCELLENT" : v >= 75 ? "ON_TRACK" : v >= 50 ? "WARNING" : v > 0 ? "CRITICAL" : "NO_COVERAGE";
...
export default function VaccinationVaccineReport() {
  ...
  const status = statusForCoverage(coverage);
  const chart = useMemo(() => [...(report?.counties || [])].sort((a, b) => Number(b.coverage_percent || 0) - Number(a.coverage_percent || 0)).slice(0, 12).map(c => ({ ...c, label: c.county_name, coverage: Number(c.coverage_percent || 0) })), [report]);
  ...
  const tooltip = ({ active, payload, label }: any) => active && payload?.length ? <div style={{ direction: "rtl", minWidth: 145, padding: "10px 12px", borderRadius: 12, background: "#061827", border: "1px solid #21637e", boxShadow: "0 14px 35px rgba(0,0,0,.35)" }}><div style={{ color: "#9be7f5", fontWeight: 900, marginBottom: 7 }}>{label}</div><b style={{ color: "#e9f8ff" }}>{pct(Number(payload[0]?.value || 0))}</b></div> : null;
  ...
  return <div className="dashboard-page" dir="rtl">
    <header className="dashboard-header"><button className="back-button" onClick={() => navigate("/gis/kpi/vaccination")}>← بازگشت به مرکز KPI</button><div className="detail-title"><span className="detail-eyebrow">گزارش تحلیلی واکسن</span><h1>{v?.disease_name || vaccineType}</h1><p>{v?.animal_group_name || animalGroup || "گروه دام"} · {v?.vaccine_brand || "برند ثبت‌شده نامشخص"}</p></div><div className="detail-status"><span>وضعیت فعلی</span><strong className={statusClass(status)}>{statusText(status)}</strong><b>{pct(coverage)}</b></div></header>
